const Sidebar = ({ experts, chats, setSelectedChat, fetchMessages, userId, user_name,  createChatRequest }) => (
      ...
      <div className="max-h-64 overflow-y-auto">
        {experts.filter(expert => !chats.some(chat => chat.user_id2 === expert.email)).map((expert) => (
          <div
            key={expert.email}
            onClick={() => {
              createChatRequest(userId, expert.email, user_name, expert.full_name, ''); // Add necessary params
            }}
            className="p-2 bg-white rounded-lg mb-2 shadow-md cursor-pointer hover:bg-gray-200"
          >
